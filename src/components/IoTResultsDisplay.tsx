...
const TIER_CONFIG = {
  safe:    { label: 'Safe',    bg: 'bg-green-500',  ring: 'ring-green-200',  text: 'text-green-700',  light: 'bg-green-50',  icon: <Shield    size={32} className="text-white" /> },
...
function AdulterantCard({ name, value }: { name: string; value: number }) {
  ...
  return (
    <div className={`rounded-2xl p-3 ${detected ? 'bg-red-50 border border-red-100' : 'bg-green-50 border border-green-100'}`}>
      ...
      <p className={`text-[9px] font-bold mt-0.5 ${detected ? 'text-red-500' : 'text-green-600'}`}>
    ...
    >
      ...
      {saveState !== 'saved' && (
        <motion.button
          ...
          className="w-full flex items-center justify-center gap-2 py-4 bg-[#1A6B4A] text-white rounded-2xl font-black text-sm tracking-tight hover:bg-[#145339] transition-colors disabled:opacity-60 shadow-lg shadow-green-900/20"
        >
          {saveState === 'saving' ? (
      ...
      {saveState === 'saved' && (
        <div className="flex items-center justify-center gap-2 py-3 bg-green-50 rounded-2xl text-green-700 text-sm font-black">
      ...
      <AnimatePresence>
        {!showAI ? (
          <motion.button
            ...
            className="w-full flex items-center justify-center gap-2 py-4 bg-gradient-to-r from-[#1A6B4A] to-[#1e8259] text-white rounded-2xl font-black text-sm tracking-tight hover:opacity-90 transition-opacity shadow-lg shadow-green-900/20"
          >
